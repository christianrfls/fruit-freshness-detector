...
# Image loading with image_dataset_from_directory adapted from:
...
class_names = train_data.class_names   # needed for app
print("Classes found:", class_names)

# Prefetch pattern for performance from:
# https://www.tensorflow.org/tutorials/images/classification

# purpose: keep images ready in memory so training is not slowed by disk reads.
train_data = train_data.prefetch(buffer_size=tf.data.AUTOTUNE)
test_data = test_data.prefetch(buffer_size=tf.data.AUTOTUNE)

# Transfer learning setup (MobileNetV2 base model, frozen weights, and sequential) adapted from:
# https://www.tensorflow.org/tutorials/images/transfer_learning

# MobileNetV2 pretrained on ImageNet
base_model = tf.keras.applications.MobileNetV2(
    input_shape=(224, 224, 3),
    include_top=False,  # drop its classification head, add our own
    weights="imagenet"
)
...
model.save("model/freshness_model.keras") 
